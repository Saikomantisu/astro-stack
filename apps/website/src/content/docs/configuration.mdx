...
Vanilla CSS and Tailwind each create `src/styles/global.css`. Tailwind also adds the Tailwind v4 Vite plugin and its required development dependencies.
...
Choosing `none` adds no starter content. Markdown, MDX, and Content Collections each create `src/content.config.ts` plus a starter post under `src/content/posts`, using Astro's built-in `glob()` loader. MDX additionally installs and configures Astro's official MDX integration.
