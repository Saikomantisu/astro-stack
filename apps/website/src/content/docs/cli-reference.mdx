...
<Warning title="Validate combinations before automation">
  Pages CMS requires Markdown, MDX, Content Collections, or a Blog or Documentation project. Resend and webhooks cannot use `static` deployment. VS Code and Cursor cannot be selected together. Hooks require Git, so `--hooks --no-git` fails before files are written.
</Warning>
...
### Blog with Pages CMS and Vercel
